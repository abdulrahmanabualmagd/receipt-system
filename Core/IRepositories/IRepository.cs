...
namespace Core.IRepositories
{
    public interface IRepository<T> where T : class    // Classes Only Constraint
    {
        ...
        Task<IEnumerable<T>> GetPageAsync(int pageIndex, int pageSize, Expression<Func<T, bool>> predicate, string[]? include = null);
        Task<int> GetTotalPagesAsync(int pageSize);
        Task<int> GetTotalPagesAsync(int pageSize, Expression<Func<T, bool>> predicate, string[]? include = null);
